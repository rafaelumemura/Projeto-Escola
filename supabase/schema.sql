...
begin
  ...
  if current_subscription.id is null then
    insert into public.billing_subscriptions (
      user_id, plan_key, status, activity_limit, generated_count,
      current_period_start, current_period_end, grace_ends_at,
      provider, provider_customer_id, provider_subscription_id,
      provider_product_id, provider_offer_code, last_provider_event_id,
      last_payment_transaction_id, next_charge_at, status_reason,
      cancel_at_period_end
    )
    values (
      p_user_id, p_plan_key, 'active', plan_limit, 0,
      p_started_at, effective_period_end, effective_period_end + interval '1 day',
      'hotmart', p_provider_customer_id, p_provider_subscription_id,
      p_product_id, p_offer_code, p_event_id, p_transaction_id,
      p_period_end, 'payment_approved', false
    )
    returning * into updated_subscription;
  else
    update public.billing_subscriptions
    set
      ...
      status = 'active',
      activity_limit = plan_limit,
      generated_count = case when starts_new_cycle then 0 else least(generated_count, plan_limit) end,
      current_period_start = case when starts_new_cycle then p_started_at else current_period_start end,
      current_period_end = case
  ...
  update public.profiles set plan = p_plan_key where id = p_user_id;
  return updated_subscription;
end;
...
begin
  ...
  if p_cancel_at_period_end then
    update public.billing_subscriptions
    set
      status = case when current_period_end <= p_effective_at then 'canceled' else status end,
    ...
    set
      status = p_status,
      grace_ends_at = case
        when p_status = 'past_due' then coalesce(grace_ends_at, current_period_end + interval '1 day')
        else grace_ends_at
      end,
      suspended_at = case
